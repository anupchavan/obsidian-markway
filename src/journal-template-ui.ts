...
import { validateTemplateVariables } from "./journal-template";
...
function renderPropertyRow(
	...
	const deleteButton = row.createEl("button", { cls: "clickable-icon mw-template-delete-button" });
	setIcon(deleteButton, "trash-2");
	deleteButton.setAttribute("aria-label", "Delete property");
	deleteButton.addEventListener("click", () => {
		void (async () => {
			plugin.settings.journalProperties = plugin.settings.journalProperties.filter((item) => item.id !== property.id);
			await plugin.savePluginData();
			onTemplateChanged();
			onRefresh();
		})();
